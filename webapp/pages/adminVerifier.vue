        ...
        List Of Recruiter to be Verified
          ...
          <v-list color="">
            ...
            <v-list-item
              ...
              :key="i" :disabled="recruiter_list.length === 0"
              
            >
                 <v-list-item-title>{{ recruiter_list.recruiter_id }} </v-list-item-title>
                 <v-card-actions>
          <v-spacer></v-spacer>

          <v-btn text @click="approveRecruiter(recruiter_list.recruiter_id)">Approve</v-btn>
          <v-btn color="primary" text @click="removeRecruiter_list(recruiter_list.recruiter_id)">Remove</v-btn>
        </v-card-actions>
            </v-list-item>
          </v-list>
...
export default {
    ...
    try{
    const response= await this.$axios.put(`https://cuwewf4fsg.execute-api.ap-south-1.amazonaws.com/artwrkInit/requests`,payload);
    if (response){
